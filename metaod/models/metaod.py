...
class MetaOD(object):
    def __init__(self,
        ...
        if self.learning == 'sgd':
            # self.sample_row, self.sample_col = self.train_performance.nonzero()
    ...
    def get_train_dcg(self, user_vecs, item_vecs):
        ...
        for w in range(self.ratings.shape[0]):
            ndcg_s.append(ndcg_score([self.ratings[w,:]], [np.dot(user_vecs[w,:], item_vecs.T)]))
            
        return np.mean(ndcg_s)
    
    def train(self, meta_features, valid_meta=None, n_iter=10, learning_rate=0.1, n_estimators=100, max_depth=10, max_rate=1.05, min_rate=0.1, discount=0.95, n_steps=10):
        ...
        while ctr <= n_iter:
            ...
            for w in range(self.valid_ratings.shape[0]):
                ndcg_s.append(ndcg_score([self.valid_ratings[w,:]], [np.dot(meta_valid_scaled_new[w,:], self.item_vecs.T)], k=self.n_items))
            ...
            print('ALORS Rank PCARF Sigmoid iteration', ctr, 'train', self.train_loss_[-1], 'valid', self.valid_loss_[-1], 
