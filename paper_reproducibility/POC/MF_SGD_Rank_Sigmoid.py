...
class ExplicitMFRank():
    ...
    def train(self, meta_features, valid_meta=None, n_iter=10, learning_rate=0.1, n_estimators=100, max_depth=4, rho=1.1, alpha=0.5, max_rate=1.05, min_rate=0.1, discount=0.95, n_steps=10):
        ...
        # for w in range(n_iter):
        #     learning_rate_full[w] = learning_rate_full[w] * np.power(discount,w)
            
        # print(step_size, lr_list)
        # print(lr_list_reverse)
        ...
        while ctr <= n_iter:
            ...
            print('ALORS Rank Sigmoid iteration', ctr, 'train', self.train_loss_[-1], 'valid', self.valid_loss_[-1], 
    ...
    # def train_bk(self, meta_features, n_iter=10, learning_rate=0.1, n_estimators=100, max_depth=10):
    #     """ Train model for n_iter iterations from scratch."""
    #     # initialize latent vectors        
    #     self.user_vecs = np.random.normal(scale=1./self.n_factors,\
    #                                       size=(self.n_users, self.n_factors))
...
# EMF.train(n_iter=500, meta_features=train_data_cv, valid_meta=valid_data_cv, learning_rate=0.5, max_rate=0.2)
